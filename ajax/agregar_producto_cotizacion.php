...
if($idmaterial=='0')
{
	...
	$count_query   = mysqli_query($con, "SELECT count(*) AS numrows FROM tmp_materiales where id_pedido='".$idpedido."' and session_id ='". $session_id."'");
	...
	{
		$sql_tmp =mysqli_query($con,"select DISTINCT Material_equipo.idMaterial, Material_equipo.Precio, Material_equipo.depreciacion from Material_equipo,Material_actividad, detalle_actividad Where Material_equipo.idMaterial = Material_actividad.idMaterial and Material_actividad.idactividad = detalle_actividad.id_actividad and detalle_actividad.id_pedido ='".$idpedido."'");
		while ($rw=mysqli_fetch_array($sql_tmp))
		{
			...
			$insert_tmp=mysqli_query($con, "INSERT INTO tmp_materiales ( id_pedido,id_material,cantidad_tmp,precio_tmp,depreciacion_tmp,session_id) VALUES ('$idpedido','$idmaterial','$cantidad','$precio_venta','$depreciacion','$session_id')");
	...
	if (!empty($idpedido) and !empty($idmaterial) and !empty($cantidad) and !empty($precio_venta) and !empty($depreciacion))
	{
		$count_query1  = mysqli_query($con, "SELECT count(*) AS numrows FROM tmp_materiales where id_material = ".$idmaterial." and id_pedido=".$idpedido." and session_id ='". $session_id."'");
